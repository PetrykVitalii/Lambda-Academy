...
const groupVacations = () => {
  const groupedVacations = groupBy(initData, (vacation) => vacation.user._id)

  const formattedVacations = Object.entries(groupedVacations).map(([userId, vacation]) => {
    return ({
      userId,
      name: vacation[0].user.name,
      weekendDays: vacation.map((v) => ({startDate: v.startDate, endDate: v.endDate})),
    })
  })
